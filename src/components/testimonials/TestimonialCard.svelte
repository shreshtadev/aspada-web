<script>
  ...
  // Safe default avatar
  let defaultAvatar = $derived('https://i.pravatar.cc/150?u=' + (testimonial.id || 'default'))

  let authorAvatar = $derived(testimonial.expand?.authorAvatar)
  // Construct image URL if avatar exists
  let avatarUrl = $derived(
    authorAvatar ? pb.files.getURL(authorAvatar, authorAvatar.attachment) : defaultAvatar
  )
</script>
...
>
  <div class="flex items-center gap-4 mb-6">
    ...
    />
    <div>
      <h3 class="font-bold text-slate-800 text-lg group-hover:text-[#d4af37] transition-colors">
  ...
  <div class="relative">
    <span class="absolute -top-3 -left-1 text-4xl text-[#d4af37]/10 font-serif leading-none">“</span
    >
    <p class="text-slate-600 italic relative z-10 pl-2 leading-relaxed line-clamp-4">
      {testimonial.content}
    </p>
  </div>
