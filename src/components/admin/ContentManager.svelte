<script>
  ...
  async function loadData(page) {
    ...
    try {
      ...
      allCategories = metadataRes.filter((m) => m.categoryType === 'postCategory')
  ...
  async function handleQuickAdd() {
    ...
    try {
      ...
      if (modalType === 'category') {
        allCategories = [...allCategories, record].sort((a, b) => a.title.localeCompare(b.title))
        formCategories = [...formCategories, record.id]
      } else if (modalType === 'tag') {
        allTags = [...allTags, record].sort((a, b) => a.title.localeCompare(b.title))
        formCategories = [...formCategories, record.id]
      }
  ...
  async function savePost() {
    ...
    try {
      ...
      if (featuredImageFile) {
        const urls = await uploadAttachment(`Featured - ${formTitle}`, [featuredImageFile])
        if (urls.length > 0) {
          const newId = urls[0]
          if (newId) {
      ...
      if (selectedId) {
        const updated = await pb.collection('posts').update(selectedId, formData)
        posts = posts.map((it) => (it.id === selectedId ? updated : it))
        selectPost(updated)
      } else {
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 h-[calc(100vh-100px)]">
  ...
  >
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-3">
      {#if loading}
        <div class="text-center py-10 text-slate-400">Loading posts...</div>
      {:else if posts.length === 0}
        <div class="text-center py-10 text-slate-400">No posts found.</div>
      {:else}
        {#each posts as p}
          <div
            ...
            <div class="flex justify-between items-start">
              <div class="flex-1">
                ...
                <div class="text-xs text-slate-500 font-medium uppercase tracking-wider mt-1">
                  {p.isPublished === 'published' ? 'Published' : 'Draft'}
                </div>
  ...
  >
    ...
    <div class="space-y-6 max-w-4xl">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <label class="block md:col-span-2">
          <span class="text-sm font-bold text-slate-700 mb-1 block">Post Title</span>
          <input
      ...
      <label class="block">
        <span class="text-sm font-bold text-slate-700 mb-1 block">Content</span>
      ...
      <div class="pt-4 border-t border-slate-100">
        <div class="block">
          <span class="text-sm font-bold text-slate-700 mb-2 block">Featured Image</span>
          <div
          ...
          >
            {#if currentFeaturedImageId && !featuredImageFile}
              <div class="text-xs text-green-600 font-bold mb-2">✓ Current Image Set</div>
            {/if}
            <input
              type="file"
            ...
            <div class="pointer-events-none">
              {#if featuredImageFile}
                <span class="text-slate-900 font-bold">{featuredImageFile.name}</span>
              {:else}
    ...
    <div class="flex gap-4 mt-8 pt-6 border-t border-slate-100">
      ...
      >
        {formLoading ? 'Saving...' : selectedId ? 'Update Post' : 'Create Post'}
      </button>
